...
const TemplateSelector: React.FC<TemplateSelectorProps> = ({
  ...
  return (
    <div>
      ...
      <div className="mb-6">
        <div
          className="border-2 border-dashed border-indigo-300 rounded-lg p-6 hover:border-indigo-500 hover:shadow-md transition-all cursor-pointer bg-indigo-50"
          onClick={onFreehandSelect}
        >
          <div className="flex items-center justify-between mb-2">
            <h3 className="text-xl font-medium text-indigo-900">Freehand Story</h3>
            <span className="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-indigo-100 text-indigo-800">
              Custom
            </span>
          </div>
          <p className="text-sm text-indigo-700 mb-4">
            Create a story from scratch without using a template. Full creative freedom!
          </p>
        </div>
      </div>

      {/* Template options */}
      <h3 className="text-lg font-medium text-gray-900 mb-4">Or select a template:</h3>

...
async function extractRankingsUsingTokens(
  ...
  try {
    ...
    console.error('Error during token-based scraping:', error);
...
async function extractItemsFromPage(page: Page): Promise<ProductBoardItem[]> {
  try {
    ...
    console.error('Error extracting items from page:', error);
    return [];
  }
}
...
async function applyScrollingTechniques(page: Page, attempt: number): Promise<void> {
  try {
    ...
    console.error('Error applying scrolling techniques:', error);
  }
}
...
async function scrapeProductBoardWithTokens(
  ...
  try {
    ...
    // Set shorter timeouts for network operations
    ...
    // Reduce scroll attempts for edge functions
    for (let attempt = 0; attempt < 10 && !foundTarget && unchangedScrolls < 3; attempt++) {
    ...
    console.error('Error during browser session:', error);
    throw error;
  } finally {
    // Close the browser
    if (browser) {

...
Deno.serve(async (req) => {
  ...
  try {
    ...
    try {
      let url = 'https://api.productboard.com/initiatives';
      
      // If initiative_id is provided, get a specific initiative
      if (initiative_id) {
        url = `${url}/${initiative_id}`;
      }
      ...
      const errorResponse = {
        success: false,
        error: error.message || 'Failed to fetch ProductBoard initiatives',

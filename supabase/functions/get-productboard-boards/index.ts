...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const { api_key, board_id } = RequestSchema.parse(body);

    try {
      // Use features endpoint instead of boards
      let url = 'https://api.productboard.com/features';
      if (board_id) {
        // If board_id is provided, add it as a query parameter
        // instead of part of the URL path
      }
      ...
      const errorResponse = {
        success: false,
        error: error.message || 'Failed to fetch ProductBoard boards',

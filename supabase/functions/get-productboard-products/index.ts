...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const { api_key, product_id, params = {} } = RequestSchema.parse(body);

    try {
      let url = 'https://api.productboard.com/products';
      
      // If product_id is provided, get a specific product
      if (product_id) {
        url = `${url}/${product_id}`;
      }
      ...
      const errorResponse = {
        success: false,
        error: error.message || 'Failed to fetch ProductBoard products',
